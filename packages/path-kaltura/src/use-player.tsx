  ...
  const result = useMemo(() => {
    function validate() {
      if (!playerIdRef.current) {
        console.warn(`trying to perform action on kaltura player but no player id was provided.`);
        return false;
      }
    ...
    return {
      ...
      getPlayerDimensions: (): null | { width: number, height: number } => {
        ...
        return new KalturaPlayerUtils(playerIdRef.current).getPlayerDimensions();
      },
      playerPlay: () => {
        if (!validate()) {
          return;
